...
function onGet(content: string) {
  emit('update:modelValue', JSON.parse(content));
  emit('takeOut', JSON.parse(content));
}
...
<template>
  <HDropdown placement="bottom">
    ...
    <PubSvgIcon v-else name="i-ep-message-box" :size="16" />
    ...
    <template #dropdown>
      <div class="w-40">
        <div class="flex items-center justify-between px-4 py-3">
          ...
          <PubSvgIcon name="i-ep-plus" class="cursor-pointer text-base" @click="dialog.visible = true" />
        </div>
        <OverlayScrollbarsComponent :options="{ scrollbars: { autoHide: 'leave', autoHideDelay: 300 } }" defer class="max-h-200px">
          ...
          <div
            ...
            text-stone-5
          >
            <PubSvgIcon name="i-tabler-mood-empty" :size="40" />
            <p m-2 text-base>
              空空如也
            </p>
          </div>
